...
if (process.argv.length < 4) {
  console.error("MErkle Producer v1.0.1:[Modified from vote lab]");
  console.error("    node merkleProducer.js root <tickets-file>");
  console.error(
    "    node merkleProducer.js proof <tickets-file> <ticket-to-prove>"
  );
  process.exit(1);
}
...
// interface functions
//[MY-NOTES] receives a list of etherium addreses
function merkleRoot(items) {
  if (items.length === 0) throw "can't build merkle tree with empty items";

  //[MY-NOTES] hashes every item and returns a new list of hashs
