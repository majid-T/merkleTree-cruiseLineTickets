...
contract("Cruise Merke Tree:", (accounts) => {
  ...
  let deployerAcc = accounts[0];
  let authorizedAcc = accounts[1];
  let unAuthoirzedAcc = accounts[2];
  ...
  //  [Deployment] - Testing for deployment to be done with deployer address as owner
  it("Contract is deployed with msg.sneder as owner Merkle Root ", async () => {
    const contractOwner = await contractInstance.owner();
    assert.equal(
      contractOwner,
      deployerAcc,
      "Deployer account is not set as owner"
    );
  });
  ...
  //  [Functionality] - Testing for granting access by contract owner
  it("Contract owner can give access to an address", async () => {
    const tx = await contractInstance.giveAccess(authorizedAcc, true, {
      from: deployerAcc,
    });
    assert.ok(tx.receipt.status == true, "Transaction done successfully.");

    const givenAuth = await contractInstance.authUsers(authorizedAcc);
    assert.equal(givenAuth, true, "Access not given to right address by owner");
  });

  //  [Functionality] - Testing for granting access to fail for none owner
  it("Others can not give access to an address", async () => {
    await truffleAssert.reverts(
      contractInstance.giveAccess(unAuthoirzedAcc, true, {
        from: unAuthoirzedAcc,
      }),
      "[CruiseMerkle]: Only owner can call"
    );
  });
  ...
  //  [Functionality] - Testing for isValidTicket function to revert with unauthorized user
  it("Funciton isValidTicket revert when unauthorized user tries to call it", async () => {
    const ticketHash =
      "0x7493d5e512c7370661ed4a5dab94a5c9ee9b741eb2c3cf34c5ff9a63f4dbe6b0";
    ...
    await truffleAssert.reverts(
      contractInstance.isValidTicket(path, witnesses, ticketHash, {
        from: unAuthoirzedAcc,
      }),
